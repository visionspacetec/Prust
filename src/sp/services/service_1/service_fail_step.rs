...
impl SpacePacket<TmPacket<ServiceFailStep>> {
    ...
    pub fn new_service_1_6<T: Request>(
        ...
        SpacePacket::<TmPacket<ServiceFailStep>>::new(
            request,
            destination_id,
            packet_name,
            err_code,
    ...
    pub fn new<T: Request>(
        ...
        let data_len =
            TmPacketHeader::TM_HEADER_LEN + REQ_ID_LEN + err_data.len() + crate::sp::PEC_LEN - 1
                + STEP_ID_LEN;
        ...
        Ok(SpacePacket::<TmPacket<ServiceFailStep>> {
            primary_header,
            data: TmPacket {
                header,
    ...
    pub fn from_bytes(buffer: &[u8]) -> Result<Self, Error> {
        ...
        if sec_header.service_type != SERVICE_TYPE || sec_header.message_subtype != 6 {
            return Err(Error::InvalidPacket);
        };
